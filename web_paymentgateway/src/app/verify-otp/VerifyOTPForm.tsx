...
export default function VerifyOTPForm() {
    ...
    // Ambil params setelah mount
    const email = isMounted ? searchParams?.get('email') || '' : '';
    const type = isMounted ? searchParams?.get('type') || 'signup' : 'signup';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (type === 'signup') router.push(data.redirectTo);
            else if (type === 'forgot_password')
                router.push(`/reset-password?email=${encodeURIComponent(email)}&otp=${otp}`);
        } catch (err: unknown) {
            if (err instanceof Error) setError(err.message);
            else setError('Terjadi kesalahan tidak terduga');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleResend = async () => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(data.error || 'Gagal mengirim ulang OTP');
    ...
    if (!isMounted) return null; // jangan render sebelum mount

    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-md w-full space-y-8">
                <div>
                    <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Verifikasi OTP</h2>
                    <p className="mt-2 text-center text-sm text-gray-600">Kode OTP telah dikirim ke WhatsApp Anda</p>
                ...
                <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                    ...
                    {resendSuccess && <div className="rounded-md bg-green-50 p-4"><p className="text-sm text-green-800">OTP berhasil dikirim ulang</p></div>}
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="000000"
                        />
                        <p className="mt-1 text-xs text-gray-500 text-center">Masukkan 6 digit kode OTP</p>
                    </div>
                    ...
                    <div className="text-center">
                        <Link href="/login" className="text-sm font-medium text-gray-600 hover:text-gray-500">Kembali ke login</Link>
                    </div>
                </form>
            </div>
        </div>
